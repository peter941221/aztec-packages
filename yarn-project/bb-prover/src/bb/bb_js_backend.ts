...
import { HIDING_KERNEL_IO_PUBLIC_INPUTS_SIZE } from '@aztec/constants';
...
export interface BBJsApi {
  ...
  /** Generate an AVM proof from serialized inputs. Returns proof fields and verification result. */
  generateAvmProof(inputs: Uint8Array): Promise<{ proof: Uint8Array[]; verified: boolean; durationMs: number }>;
  ...
  checkAvmCircuit(inputs: Uint8Array): Promise<{ passed: boolean; durationMs: number }>;
...
export class BBJsInstance implements BBJsApi {
  ...
  async generateAvmProof(inputs: Uint8Array): Promise<{ proof: Uint8Array[]; verified: boolean; durationMs: number }> {
    const timer = new Timer();
    const result = await this.api.avmProve({ inputs });
    return { proof: result.proof, verified: result.verified, durationMs: timer.ms() };
  }
  ...
  /** Check the AVM circuit from serialized inputs. */
  async checkAvmCircuit(inputs: Uint8Array): Promise<{ passed: boolean; durationMs: number }> {
    const timer = new Timer();
    const result = await this.api.avmCheckCircuit({ inputs });
    return { passed: result.passed, durationMs: timer.ms() };
  }
 ...
 *
 * Fixed goblin sub-proof sizes are verified by static_asserts in mock_verifier_inputs.test.cpp.
 */
function splitChonkProofToStructured(fields: Uint8Array[], numCustomPublicInputs: number): ChonkProof {
  // Sub-proof sizes matching C++ ChonkProof::from_field_elements().
  // Fixed sizes verified by static_asserts in mock_verifier_inputs.test.cpp.
  const HIDING_OINK_LENGTH = 96; // ProofLength::Oink<MegaZKFlavor>::LENGTH_WITHOUT_PUB_INPUTS
  const MERGE_PROOF_SIZE = 42;
  const ECCVM_PROOF_SIZE = 608;
  const IPA_PROOF_SIZE = 64;
  const JOINT_PROOF_LENGTH = 492; // TranslatorFlavor::COMMITTED_SUMCHECK_PROOF_LENGTH + MegaZKFlavor::NUM_ALL_ENTITIES

  let offset = 0;

  // Hiding oink proof = public inputs (HidingKernelIO + custom) prepended to the oink proof
  const oinkSize = HIDING_OINK_LENGTH + HIDING_KERNEL_IO_PUBLIC_INPUTS_SIZE + numCustomPublicInputs;
  const hidingOinkProof = fields.slice(offset, offset + oinkSize);
  offset += oinkSize;

  const mergeProof = fields.slice(offset, offset + MERGE_PROOF_SIZE);
  offset += MERGE_PROOF_SIZE;

  const eccvmProof = fields.slice(offset, offset + ECCVM_PROOF_SIZE);
  offset += ECCVM_PROOF_SIZE;

  const ipaProof = fields.slice(offset, offset + IPA_PROOF_SIZE);
  offset += IPA_PROOF_SIZE;
