...
export class BBCircuitVerifier implements ClientProtocolCircuitVerifier {
  ...
  public async verifyProof(tx: Tx): Promise<IVCProofVerificationResult> {
    ...
    try {
      ...
      const verificationKey = this.getVerificationKeyData(circuit);
      const numCustomPublicInputs = verificationKey.numPublicInputs - HIDING_KERNEL_IO_PUBLIC_INPUTS_SIZE;
      ...
      const { verified, durationMs } = await this.bbJsFactory.withVerifierInstance(instance =>
        instance.verifyChonkProof(fieldsAsBuffers, verificationKey.keyAsBytes, numCustomPublicInputs),
      );
