...
import { BB_RESULT } from '../../bb/execute.js';
...
export class BBNativeRollupProver implements ServerCircuitProver {
  ...
  private async createAvmProof(
    ...
    const {
      proof: proofFieldArrays,
      verified,
      durationMs,
    } = await this.bbJsFactory.withFreshInstance(instance => instance.generateAvmProof(inputsBuffer));

    if (!verified) {
      throw new ProvingError('AVM proof verification failed after proving');
    }
  ...
  public async verifyProof(circuitType: ServerProtocolArtifact, proof: Proof) {
    ...
    if (!verified) {
      throw new ProvingError(
        'Failed to verify proof from key!',
        { status: BB_RESULT.FAILURE, reason: 'Verification failed' },
        false,
      );
    }
