...
describe('KVArchiverDataStore', () => {
  ...
  describe('addCheckpoints', () => {
    ...
    it('throws an error if the previous block does not exist in the store', async () => {
      ...
      await expect(store.getCheckpointedBlock(BlockNumber(1))).resolves.toBeUndefined();
    });
    ...
    it('throws an error if block has invalid checkpoint index', async () => {
      ...
      await expect(store.addCheckpoints([publishedCheckpoint])).rejects.toThrow(BlockIndexNotSequentialError);
      await expect(store.getCheckpointedBlock(BlockNumber(BlockNumber(1)))).resolves.toBeUndefined();
    });
  ...
  describe('multi-block checkpoints', () => {
    ...
    it('removing multiple checkpoints with multiple blocks in one go', async () => {
      ...
      for (let blockNumber = 1; blockNumber <= 5; blockNumber++) {
        expect(await store.getCheckpointedBlock(BlockNumber(BlockNumber(blockNumber)))).toBeDefined();
      }
    ...
    it('getCheckpointedBlock returns correct checkpoint info for blocks within multi-block checkpoints', async () => {
      ...
      for (let i = 0; i < 3; i++) {
        const blockNumber = i + 1;
        const retrievedBlock = await store.getCheckpointedBlock(BlockNumber(BlockNumber(blockNumber)));
    ...
    it('removing a multi-block checkpoint removes all its blocks', async () => {
      ...
      for (let blockNumber = 1; blockNumber <= 3; blockNumber++) {
        expect(await store.getCheckpointedBlock(BlockNumber(blockNumber))).toBeUndefined();
      }
  ...
  describe('uncheckpointed blocks', () => {
    ...
    it('getBlock retrieves uncheckpointed blocks', async () => {
      ...
      expect((await store.getBlock(BlockNumber(2)))?.number).toBe(2);
      ...
      const retrieved4 = await store.getBlock(BlockNumber(4));
      expect(retrieved4!.number).toBe(4);
      expect(retrieved4!.equals(block4)).toBe(true);
      const retrieved5 = await store.getBlock(BlockNumber(5));
    ...
    it('getCheckpointedBlock returns undefined for uncheckpointed blocks', async () => {
      ...
      expect((await store.getCheckpointedBlock(BlockNumber(2)))?.block.number).toBe(2);

      // getCheckpointedBlock should return undefined for uncheckpointed blocks
      expect(await store.getCheckpointedBlock(BlockNumber(3))).toBeUndefined();
      expect(await store.getCheckpointedBlock(BlockNumber(4))).toBeUndefined();
    ...
    it('checkpoint adopts previously added uncheckpointed blocks', async () => {
      ...
      expect(await store.getCheckpointedBlock(BlockNumber(1))).toBeUndefined();
      expect(await store.getCheckpointedBlock(BlockNumber(2))).toBeUndefined();
      expect(await store.getCheckpointedBlock(BlockNumber(3))).toBeUndefined();
      ...
      const checkpointed1 = await store.getCheckpointedBlock(BlockNumber(1));
      ...
      const checkpointed2 = await store.getCheckpointedBlock(BlockNumber(2));
      expect(checkpointed2).toBeDefined();
      expect(checkpointed2!.checkpointNumber).toBe(1);

      const checkpointed3 = await store.getCheckpointedBlock(BlockNumber(3));
    ...
    it('can add more uncheckpointed blocks after a checkpoint and then checkpoint them', async () => {
      ...
      expect(await store.getCheckpointedBlock(BlockNumber(3))).toBeUndefined();
      expect(await store.getCheckpointedBlock(BlockNumber(4))).toBeUndefined();
      expect(await store.getCheckpointedBlock(BlockNumber(5))).toBeUndefined();
      ...
      const checkpointed4 = await store.getCheckpointedBlock(BlockNumber(4));
      expect(checkpointed4).toBeDefined();
      expect(checkpointed4!.checkpointNumber).toBe(2);

      const checkpointed5 = await store.getCheckpointedBlock(BlockNumber(5));
  ...
  describe('addProposedBlock validation', () => {
    ...
    it('allows blocks for the initial checkpoint when store is empty', async () => {
      ...
      expect((await store.getBlock(BlockNumber(2)))?.equals(block2)).toBe(true);
      expect(await store.getLatestBlockNumber()).toBe(2);
    });
  ...
  describe('getCheckpointedBlock', () => {
    ...
    it.each(blockNumberTests)('retrieves previously stored block %i', async (blockNumber, getExpectedBlock) => {
      const retrievedBlock = await store.getCheckpointedBlock(BlockNumber(blockNumber));
    ...
    it('returns undefined if block is not found', async () => {
      await expect(store.getCheckpointedBlock(BlockNumber(12))).resolves.toBeUndefined();
    });

    it('returns undefined for block number 0', async () => {
      await expect(store.getCheckpointedBlock(BlockNumber(0))).resolves.toBeUndefined();
    });
  });
  ...
  describe('pendingCheckpoint', () => {
    ...
    it('clears pending checkpoint when confirmed checkpoints are added', async () => {
      // Add checkpoint 1
      const checkpoint1 = makePublishedCheckpoint(
        await Checkpoint.random(CheckpointNumber(1), { numBlocks: 1, startBlockNumber: 1 }),
        10,
      ...
      expect(await store.blockStore.getPendingCheckpointNumber()).toBe(2);
    ...
    it('clears pending checkpoint when checkpoints are removed past it', async () => {
    ...
    it('does not clear pending checkpoint when removing checkpoints before it', async () => {
      ...
      expect(await store.blockStore.getPendingCheckpointNumber()).toBe(3);
    });

    it('allows addProposedBlocks when pending checkpoint matches expected', async () => {
      // Add checkpoint 1
      const checkpoint1 = makePublishedCheckpoint(
        await Checkpoint.random(CheckpointNumber(1), { numBlocks: 1, startBlockNumber: 1 }),
        10,
