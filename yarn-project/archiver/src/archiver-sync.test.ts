...
import type { PendingCheckpointData } from '@aztec/stdlib/checkpoint';
...
describe('Archiver Sync', () => {
  ...
  describe('checkpointing local proposed blocks', () => {
    ...
    it('does not prune blocks covered by a pending checkpoint in current slot', async () => {
      ...
      // Set a pending checkpoint covering these blocks (simulating pipelining)
      const pendingCheckpoint: PendingCheckpointData = {
      ...
      await archiver.setPendingCheckpoint(pendingCheckpoint);

      // Advance L1 to block 2 (still in slot 1) — pending checkpoint is still current
      fake.setL1BlockNumber(2n);
      await archiver.syncImmediate();
      ...
      // Pending checkpoint should still be set
      expect(await archiverStore.blockStore.getPendingCheckpoint()).toBeDefined();
    }, 15_000);
    ...
    it('prunes blocks and clears stale pending checkpoint when slot ends', async () => {
      ...
      // Set a pending checkpoint and pipelining tree in progress (simulating pipelining)
      const pendingCheckpoint: PendingCheckpointData = {
        checkpointNumber: CheckpointNumber(2),
        header: CheckpointHeader.empty({ slotNumber: SlotNumber(1) }),
        startBlock: BlockNumber(lastBlockInCheckpoint1 + 1),
      ...
      await archiver.setPendingCheckpoint(pendingCheckpoint);
      await archiver.setPipeliningTreeInProgress(2n);
      ...
      expect(await archiverStore.blockStore.getPendingCheckpoint()).toBeUndefined();
    }, 15_000);
  });
});
