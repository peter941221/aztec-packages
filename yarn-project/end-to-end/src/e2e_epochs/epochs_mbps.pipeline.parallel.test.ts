...
describe('e2e_epochs/epochs_mbps_pipeline', () => {
  ...
  it('prunes uncheckpointed blocks when proposer fails to deliver', async () => {
    ...
    // Wait for at least 1 checkpoint on L1
    ...
    const newCheckpointNumber = await archiver.getCheckpointNumber();

    // Wait for a new checkpoint (recovery)
    const nextCheckpointNumber = CheckpointNumber(Number(newCheckpointNumber) + 1);
    const recoveryTimeout = test.L2_SLOT_DURATION_IN_S * test.epochDuration * 4;
    logger.warn(`Waiting for recovery checkpoint ${nextCheckpointNumber} (timeout=${recoveryTimeout}s)`);
    await test.waitUntilCheckpointNumber(nextCheckpointNumber, recoveryTimeout);
