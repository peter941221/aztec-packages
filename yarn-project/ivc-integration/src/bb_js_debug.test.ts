 ...
 */
import { BBJsInstance, type BBJsProofResult, executeBB } from '@aztec/bb-prover';
...
describe('BB.js Debug Wrapper', () => {
  ...
  it('CLI bb prove reproduces the same proof as bb.js', async () => {
    ...
    expect(result.status).toBe(0); // BB_RESULT.SUCCESS
  ...
  it('CLI bb verify succeeds with debug output files', async () => {
    ...
    const verifyResult = await executeBB(
      BB_PATH,
      'verify',
      [
        '--scheme',
