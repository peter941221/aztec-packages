...
AvmProveResult avm_prove_from_bytes(std::vector<uint8_t> inputs)
{
    ...
    // NOTE: Temporarily we also verify after proving (matching avm_prove behavior).
    info("verifying...");
    bool verified = avm.verify(proof, proving_inputs.public_inputs);
    info("verification: ", verified ? "success" : "failure");

    return AvmProveResult{ .proof = std::move(proof), .verified = verified };
}
