...
graph LR
    ...
    style ERC20 fill:#f0f0f0,stroke:#999,stroke-width:2px
```
...
The bridge reuses the existing `Token` contract and the `token_portal_content_hash_lib` for content hash functions. Add these dependencies to `contract/Nargo.toml`:
...
The bridge stores two things: the L2 token address and the L1 portal address. First, create the config module at `contract/src/config.nr`:

#include_code config /docs/examples/contracts/aave_bridge/src/config.nr rust

Then replace `contract/src/main.nr`:
...
#include_code mock_erc20 /docs/examples/solidity/aave_bridge/MockERC20.sol solidity

`contracts/MockAToken.sol` — Aave's yield-bearing token mock:

#include_code mock_atoken /docs/examples/solidity/aave_bridge/MockAToken.sol solidity

`contracts/MockAavePool.sol` — simplified Aave lending pool that returns a configurable yield:

#include_code mock_aave_pool /docs/examples/solidity/aave_bridge/MockAavePool.sol solidity
...
The integration script imports Solidity ABIs from `target/solidity/aave_bridge/`. Hardhat compiles to `artifacts/contracts/` by default. Copy the compiled artifacts so the script can find them:

```bash
mkdir -p target/solidity/aave_bridge
cp -r artifacts/contracts/*.sol target/solidity/aave_bridge/
```
:::
...
#include_code setup /docs/examples/ts/aave_bridge/index.ts typescript
...
### "Block not yet proven" — script hangs waiting for proof

The deposit flow waits for the L2 block containing your exit transaction to be proven on L1. On the local sandbox, proving typically takes 30–60 seconds. If it takes longer, check that your local network is running and producing blocks.
