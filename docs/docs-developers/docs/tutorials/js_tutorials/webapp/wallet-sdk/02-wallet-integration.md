...
Define your wallet's identity in a config object with `walletId`, `name`, `icon`, and `chainId`. See [`test-extension/src/config.ts`](https://github.com/AztecProtocol/aztec-packages/tree/master/docs/examples/webapp-tutorial/test-extension/src/config.ts).
