...
#
# Available examples: connection, getting_started, advanced, authwit, testing, aave_bridge
...
if [ $# -eq 0 ]; then
    EXAMPLES=("aztecjs_connection" "aztecjs_getting_started" "aztecjs_advanced" "aztecjs_authwit" "aztecjs_testing" "aave_bridge")
else
    EXAMPLES=()
    for arg in "$@"; do
        case "$arg" in
